...
class NoSplitTrack:
    ...
    """
    def __init__(self, J, J_plus, lab_J=None, lab_J_plus=None, lambda_qua=10, lambda_ovr=10,
                 lambda_nst=10, lambda_nfl=10, target_window_len=50, growth_rate=1.3,
                 number_of_epoch=5000, temperature=1000, dt=.995, epsilon=.005):
        """
        ...
        for cell in self.J.cells.values():
            ...
            poss_cells = np.unique(self.J_plus.labeled_image[cell.cell_center[1]-self.tw_len:
                                                             cell.cell_center[1]+self.tw_len,
                                   cell.cell_center[0]-self.tw_len:cell.cell_center[0]+self.tw_len])[1:]
    ...
    def make_lieklihood_table(self):
        """
...
class SplitTrack:
    ...
    def __init__(self, J, J_plus, lab_J=None, lab_J_plus=None,
                 lam1=.01, lam2=.0001, lam3=500, lam4=.05, epoch_num=5000,
                 temperature=1000, dt=.995, epsilon=.005):
        """
    ...
    def length_cost(self):
        return sum([(self.minimum_length - np.linalg.norm(np.array(self.J_plus.cells[y].end1) -
                                                          np.array(self.J_plus.cells[y].end2)))**2
